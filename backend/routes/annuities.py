...
def calc_annuity_fv(req: AnnuityRequest):
    return {"result": future_value_annuity(req.A, req.n, req.i)}

@router.post("/pv")
def calc_annuity_pv(req: AnnuityRequest):
    return {"result": present_value_annuity(req.A, req.n, req.i)}

@router.post("/from-fv")
def calc_annuity_from_fv(req: AnnuityRequest):
    return {"result": annuity_from_future_value(req.F, req.n, req.i)}

@router.post("/from-pv")
def calc_annuity_from_pv(req: AnnuityRequest):
    return {"result": annuity_from_present_value(req.P, req.n, req.i)}

@router.post("/periods-from-pv")
def calc_periods_from_pv(req: AnnuityRequest):
    return {"result": periods_from_present_value_annuity(req.P, req.A, req.i)}

@router.post("/periods-from-fv")
def calc_periods_from_fv(req: AnnuityRequest):
    return {"result": periods_from_future_value_annuity(req.F, req.A, req.i)}

@router.post("/perpetuity")
def calc_perpetuity(req: AnnuityRequest):
    return {"result": perpetuity_present_value(req.A, req.i)}
